...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`favorites_product_idx\` ON \`favorites\` (\`product_id\`);`)
  ...
  await db.run(sql`CREATE UNIQUE INDEX \`user_product_idx\` ON \`favorites\` (\`user_id\`,\`product_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`banners\` (
  	...
  	\`tag\` text,
  	\`description\` text,
  	\`offer_title\` text DEFAULT 'Hasta',
  	\`offer_value\` text,
  	\`price\` text,
  	\`button_style\` text DEFAULT 'btn-primary',
  	\`button_text\` text,
  	\`button_link\` text,
  	\`second_button_text\` text,
  	\`second_button_link\` text,
  	\`second_button_style\` text DEFAULT 'btn-glass-strong',
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`subcategories_slug_idx\` ON \`subcategories\` (\`slug\`);`)
